...
const ChatPage = () => {
 ...
 const { messages, getMessage, subscribeToMessage, unSubscribeFromMessage } = useMessageStore();
 const { authUser } = useAuthStore();
 const { matches, getMyMatches, isLoadingMyMatch } = useMatchStore();
 ...
 }, [getMyMatches, authUser, getMessage, subscribeToMessage, unSubscribeFromMessage, id])

 if(isLoadingMyMatch) return <LoadingMessagesUI />

 if(!match){
  return <MatchNotFound />
 }

  return <div className="flex flex-col h-screen bg-gray-100 bg-opacity-50">
