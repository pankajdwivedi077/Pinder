...
const Sidebar = () => {
  ...
  const { matches, isLoadingMyMatch, getMyMatches } = useMatchStore();
  ...
  return (
    <>
      ...
      <div
        className={`
          fixed inset-y-0 left-0 z-30 w-64 max-w-[75%] bg-white shadow-md transition-transform duration-300
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="flex-grow overflow-y-auto p-4">
            {isLoadingMyMatch ? (
              ...
              matches.map((match) => (
                <Link key={match._id} to={`/chat/${match._id}`} onClick={toggleSidebar}>
      ...
      {/* Sidebar Toggle Button (Ensures it's always visible) */}
      <button
        className="lg:hidden fixed top-4 left-4 p-2 bg-pink-500 text-white rounded-md z-40"
        onClick={toggleSidebar}
      >
